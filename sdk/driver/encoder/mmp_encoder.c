...
#ifdef STREAM_RING_BUF
    #define sRingBufSize 3500 * 2048
static MMP_UINT8  *sRingBufHead;
static MMP_UINT8  *sRingBufCurPos;
static MMP_UINT32 sRingBufRemain;
#endif
    ...
    AVC_ENCODER *ptEncoder)
{
...
#ifdef STREAM_RING_BUF
    if (ptEncoder->bEnableRingBuffer)
    {
        sRingBufHead                = sRingBufCurPos = ptEncoder->pStreamBufAdr[0];
        sRingBufRemain              = sRingBufSize;
        ptEncoder->pStreamBufAdr[0] = ptEncoder->pStreamBufAdr[1] = sRingBufHead;
    }
#endif
    ...
    if ((error = encoder_memory_GetPSBuffer(&ptmpBufAdr, ptEncoder->InstanceNum)) == VIDEO_ERROR_SUCCESS)
    {
        ...
        ptEncoder->pHdrBufAddr[2] = (MMP_UINT8 *) ((MMP_UINT32)ptmpBufAdr + 18 * 1024);
    }
    else
        goto RETURN_ERROR;
    ...
    AVC_ENCODER *ptEncoder)
{
    ...
    if (ptEncoder->bMP4format == MMP_TRUE)
    {
        ...
        encoder_CreateMP4SEI(ptEncoder, (MMP_UINT32 *)ptEncoder->pHdrBufAddr[1], 8 * 1024);
    }
    else
    {
        ...
        encoder_CreatePFrameHdr(ptEncoder, (MMP_UINT32 *)ptEncoder->pHdrBufAddr[1], 8 * 1024);
    }
#endif
    // create instance success
    ...
    MMP_BOOL *bFrameEnd)
{
    ...
    size       = (VPU_ReadRegister(BIT_WR_PTR) - VPU_ReadRegister(BIT_RD_PTR)) + offset;
    *StreamLen = size;

#ifdef STREAM_RING_BUF
    if (ptEncoder->bEnableRingBuffer)
    {
        sRingBufRemain -= (((size + ((1 << (3)) - 1)) >> (3)) << (3));
        sRingBufCurPos  = (MMP_UINT8 *) ((MMP_UINT32) sRingBufCurPos + (((size + ((1 << (3)) - 1)) >> (3)) << (3)));

        if (sRingBufRemain >= 1000 * 1024)
        {
            ptEncoder->pStreamBufAdr[(ptEncoder->streamBufSelect + 1) % (ptEncoder->streamBufCount)] = sRingBufCurPos;
        }
        else
        {
            ptEncoder->pStreamBufAdr[(ptEncoder->streamBufSelect + 1) % (ptEncoder->streamBufCount)] = sRingBufHead;
            sRingBufRemain                                                                           = sRingBufSize;
            sRingBufCurPos                                                                           = sRingBufHead;
        }
    }
#endif
