...
#define DEF_MICVOLGAINDB         10 //kenny
#define DEF_MICBOOSTGAIN         1
#define DEF_HDMIBOOSTGAIN        7 //kenny            //0db
...
static MMP_MUTEX         wsmutex;

void Delay(MMP_UINT8 time)
{ 
   unsigned int  i,j,k; 

   for(i=0; i<time; i++) asm volatile("");
}
...
// write command to ws106    
void WriteCommand(unsigned char a,unsigned char b,unsigned char c)
{
	 SETSCL(0);
	 Delay(200);
    SETSDADIR(MMP_TRUE);   //sda Êä³ö
	 Delay(200);
    Start();      
    Send(a); 
    Send(b); 
...
// read command from ws106
void ReadCommand(unsigned char  *pt,unsigned char count)
{
	 SETSDADIR(MMP_FALSE);
	 SETSCL(0);
    Delay(200);
...
// read eeprom
void ReadEEPROM(unsigned char addr,unsigned char  *pt,unsigned char count)
{
	unsigned char i;
    Action();
    WriteCommand(0x30,addr,0xff);
    ReadCommand(pt,count);
    for(i = 0; i < 8; i++)
    	Action();
}

// write eeprom
void WriteEEPROM(unsigned char addr,unsigned char  *pt)
{
    WriteCommand(0x38, addr, *pt);
    Action();
}


// check
unsigned char Check(unsigned char  *pt)
{
     unsigned char temp[4];
     unsigned char i = MMP_FALSE;
     unsigned char k = 0x80;
	  
	  Action();
     WriteCommand(0x31,0xff,0xff);
     ReadCommand(temp, 4);
     	if((temp[0] & 0xFF) != 0x00)
   {
		for(i = 0; i < 8; i++)
...
ccc_1:			
		 WriteCommand(0x39,0,temp[0]);
        Action();
        for(i = 1; i < 4; i++, pt++)
        {
             WriteCommand(0x33,i,*pt);
             Action();
        }
        WriteCommand(0x39,0,0x7F);
        Action();
        WriteCommand(0x31,0xff,0xff);
        ReadCommand(temp, 4);
        if(temp[0] == 0x7F)
...
static unsigned char fact[]={0X1F,0X3D,0X61,0X57,0X84,0XA8};
static unsigned char custom[]={0X2f,0Xc3,0X95,0X86,0X1a,0X34};
static unsigned char key[]={0X5b,0X32,0X7c};
...
#ifdef ENABLE_ENCRYPTION
    if (MMP_NULL == wsmutex)
        wsmutex = PalCreateMutex(MMP_NULL);
	
    PalWaitMutex(wsmutex, 1000);
    printf("WS106 Read Command 0x%d Bytes\r\n",NByte);
    ReadCommand(pData,NByte);
    PalReleaseMutex(wsmutex);	
#endif
    ...
    unsigned int  NByte)
{
    ...
    switch(pData[0])
    {
	 ...
	 case 0x02:  //write command
	      WriteCommand(pData[1],pData[2],pData[3]);
		printf("WS106 Write Command 0x%x,0x%x,0x%x\r\n",pData[1],pData[2],pData[3]);
		 break;
 
	 case 0x03:  //command 3
              for(i = 0; i < 8; i++)
             	Action();
	      printf("WS106 Action 8 \r\n");
		break;
...
MMP_BOOL WS106_Encryption(void)
{
     unsigned char readtmp1[6],writetmp[5],i;
#ifdef  ENABLE_ENCRYPTION
       {  // read eprom address 0x00,factory ID
	   writetmp[0] = 0x01;  //command 1
	   write_ws106_command(writetmp,4);
	   writetmp[0] = 0x02;//command 2
	   writetmp[1] = 0x30; // read eprom
	   writetmp[2] = 0x00; //address
	   writetmp[3] = 0xff; //true
	   write_ws106_command(writetmp,4);
	   read_ws106_command(readtmp1,6);
	   writetmp[0] = 0x03;//commad 3
	   write_ws106_command(writetmp,4);
	//   printf("read ws106 factory ID\r\n");
	  for(i=0;i<6;i++)
	  {
  	    // printf("+%x+",readtmp1[i]);	
 	     //printf("\r\n");	  
	      if(readtmp1[i] != fact[i]) return MMP_FALSE;
	  }
	}

	{  //read eprom address ox10,customer ID
	     writetmp[0] = 0x01;
	     write_ws106_command(writetmp,4);
	     writetmp[0] = 0x02;
	     writetmp[1] = 0x30;
	     ...
	     writetmp[0] = 0x03;
	     write_ws106_command(writetmp,4);
	   //  printf("read ws106 customer ID\r\n");
	     for(i=0;i<6;i++)
	     {
 	     //   printf("-%x-",readtmp1[i]);	
	     //   printf("\r\n");
	       if(readtmp1[i] != custom[i]) return MMP_FALSE;
	     }
	}

       for(i = 0; i < 2; i++)
       {
            if(Check(key))return MMP_TRUE;
       }  
        ...
        if (PalGetDuration(CheckDeviceClock) > 500)
        {
            ...
            if (avSyncIsVideoStable())
            {
                ...
                if (gbAutoChange == MMP_TRUE)
                {
                    ...
                    if (stableSource == CAPTURE_VIDEO_SOURCE_HDMI)
                    {
                        ...
                        gtSystemStatus &= (~SYSTEM_YPBPR_INPUT);
                        gtSystemStatus |= SYSTEM_DIGITAL_INPUT;
                        dbg_msg(DBG_MSG_TYPE_INFO, "auto: HDMI\r\n");
                    }
                    else if (stableSource == CAPTURE_VIDEO_SOURCE_YPBPR)
                    {
                        gtSystemStatus &= (~SYSTEM_DIGITAL_INPUT);
                        gtSystemStatus &= (~SYSTEM_CVBS_INPUT);
                        gtSystemStatus |= SYSTEM_YPBPR_INPUT;
                        dbg_msg(DBG_MSG_TYPE_INFO, "auto: YPBPR\r\n");
                    }
                    else if (stableSource == CAPTURE_VIDEO_SOURCE_CVBS)
                    {
                        ...
                        gtSystemStatus &= (~SYSTEM_YPBPR_INPUT);
                        gtSystemStatus |= SYSTEM_CVBS_INPUT;
                        dbg_msg(DBG_MSG_TYPE_INFO, "auto: CVBS\r\n");
                    }
                }
        #endif
                if (_IsContentProtection())
            ...
            if (gbChangeDevice || gbHDMIRXReboot)
            {
                ...
                if (gbHDMIRXReboot && (CapDev == CAPTURE_VIDEO_SOURCE_HDMI))
                {
                    ...
                    dbg_msg(DBG_MSG_TYPE_INFO, "----1. HDMIRX---");
                }
        #ifdef AUTO_SWITCH_SOURCE
                if (gbAutoChange == MMP_TRUE)
                {
                    if (CapDev == CAPTURE_VIDEO_SOURCE_HDMI)
                    {
                        ...
                        dbg_msg(DBG_MSG_TYPE_INFO, "----2.CAT9883---");
                    }
                    else if (CapDev == CAPTURE_VIDEO_SOURCE_YPBPR)
                    {
                        newCapDev = CAPTURE_VIDEO_SOURCE_CVBS;
                        dbg_msg(DBG_MSG_TYPE_INFO, "----2.ADV7180---");
                    }
                    else if (CapDev == CAPTURE_VIDEO_SOURCE_CVBS)
                    {
                        newCapDev = CAPTURE_VIDEO_SOURCE_HDMI;
                        dbg_msg(DBG_MSG_TYPE_INFO, "----2. HDMIRX---");
                    }
                }
        #endif
        #ifdef INPUT_SWITCH_YPBPR_CVBS
                if (gbAutoChange == MMP_FALSE)
                {
                    ...
                    {
                        newCapDev = CAPTURE_VIDEO_SOURCE_YPBPR;
                        dbg_msg(DBG_MSG_TYPE_INFO, "----CAT9883---");
                    }

                    if (gtSystemStatus & SYSTEM_CVBS_INPUT)
                    {
                        newCapDev = CAPTURE_VIDEO_SOURCE_CVBS;
                        dbg_msg(DBG_MSG_TYPE_INFO, "----ADV7180---");
                    }
                    if (gtSystemStatus & SYSTEM_DIGITAL_INPUT)
                    {
                        newCapDev = CAPTURE_VIDEO_SOURCE_HDMI;
                        dbg_msg(DBG_MSG_TYPE_INFO, "----HDMIRX---");
                    }
                }
        #endif
                gtSystemStatus |= SYSTEM_UNSTABLE_ERROR;
    ...
    for (;;)
    {
        ...
        if (buttonPollingTime == 0)
        {
...
#ifdef SUPPORT_IR_CONTROL
            {
                ...
                if ((tempkey & 0xFFFF) == 0xec00)
                {
                    tempkey = (tempkey >> 16) & 0xFFFF;
                    if (tempkey == 0xe817 ||tempkey == 0x17e8)
                        {
                            key = KEY_CODE_S4;       
			    printf("key 4 snap\r\n");
                    	}
        ...
        if ((gtSystemStatus & STSTEM_STAND_ALONE_MODE))
        {
            ...
            if (gtSystemStatus & SYSTEM_STARTUP_LED)
            {
                ...
                if (gtSystemStatus & SYSTEM_YPBPR_INPUT)
                {
                    sync = MMP_TRUE;
                    /*
                        if (mmpCapGetCaptureDevice() == MMP_CAP_DEV_ADV7180)
                        {
                            if(gtSystemStatus & statusMaskOn)
                            {
                                gtLedCtrl.EnHDMI = MMP_FALSE;
...
#ifdef TIME_FILENAME//kenny 20150825 def EXTERNAL_RTC
   // mmpExRtcGetDate(&gYear, &gMonth, &gDay);
  //  mmpExRtcGetTime(&gHour, &gMin, &gSec);
