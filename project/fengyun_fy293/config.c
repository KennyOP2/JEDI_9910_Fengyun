...
MMP_UINT32
config_set_dev_type(
    PAL_DEVICE_TYPE type)
{
    _LOCK();
    gtConfig.devicetype = type;
    _UNLOCK();
    _CONFIG_STORE();
    return MMP_SUCCESS;
}
...
{
    _LOCK();
    *type = gtConfig.devicetype;
    _UNLOCK();
    return MMP_SUCCESS;
}

MMP_UINT32
config_set_record_resolution(
    RECORD_RESOLUTION resolution)
{
    _LOCK();
    gtConfig.resolution = resolution;
    _UNLOCK();
    _CONFIG_STORE();
    return MMP_SUCCESS;
}
...
MMP_UINT32
config_get_record_resolution(
    RECORD_RESOLUTION *resolution)
{
    _LOCK();
    *resolution = gtConfig.resolution;
    _UNLOCK();
    return MMP_SUCCESS;
}
...
{
    _LOCK();
    if (is_pc_connect_mode)
        PalMemcpy(&gtConfig.tVideoEnPara_pc[devnum][index], &config.tVideoEnPara[devnum][index], sizeof(VIDEO_ENCODER_PARAMETER));
    else
...
config_load()
{
    ...
    if (MMP_SUCCESS != result)
    {
        ...
        gtConfig.digitalvolume                             = DIGITAL_AUDIO_DEFAULT;
        ...
        gtConfig.disableHDCP                               = 0;     //default hdcp on;
        gtConfig.devicetype                                = PAL_DEVICE_TYPE_USB0;
        gtConfig.resolution                                = SD_RESOLUTION;
        _set_default_videncodepara();
        _UNLOCK();
        _CONFIG_STORE();
        return result;
    ...
    void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            if (coreIsPCConnectMode())
            {
                gptVideoEnPara[i][index]->EnWidth         = tNorConfig->tVideoEnPara_pc[i][index].EnWidth;
                gptVideoEnPara[i][index]->EnHeight        = tNorConfig->tVideoEnPara_pc[i][index].EnHeight;
                gptVideoEnPara[i][index]->EnBitrate       = tNorConfig->tVideoEnPara_pc[i][index].EnBitrate;
                gptVideoEnPara[i][index]->EnDeinterlaceOn = tNorConfig->tVideoEnPara_pc[i][index].EnDeinterlaceOn;
                gptVideoEnPara[i][index]->EnFrameDouble   = tNorConfig->tVideoEnPara_pc[i][index].EnFrameDouble;
                gptVideoEnPara[i][index]->EnSkipMode      = tNorConfig->tVideoEnPara_pc[i][index].EnSkipMode;
                gptVideoEnPara[i][index]->EnFrameRate     = tNorConfig->tVideoEnPara_pc[i][index].EnFrameRate;
                gptVideoEnPara[i][index]->EnGOPSize       = tNorConfig->tVideoEnPara_pc[i][index].EnGOPSize;
                gptVideoEnPara[i][index]->EnAspectRatio   = tNorConfig->tVideoEnPara_pc[i][index].EnAspectRatio;
            }
            else
            {
    ...
    VIDEO_ENCODER_PARAMETER  *ptEnPara)
{
    ...
    if (coreIsPCConnectMode())
    {
        ptEnPara->EnWidth         = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnWidth;
        ptEnPara->EnHeight        = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnHeight;
        ptEnPara->EnBitrate       = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnBitrate;
        ptEnPara->EnDeinterlaceOn = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnDeinterlaceOn;
        ptEnPara->EnFrameDouble   = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnFrameDouble;
        ptEnPara->EnSkipMode      = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnSkipMode;
        ptEnPara->EnFrameRate     = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnFrameRate;
        ptEnPara->EnGOPSize       = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnGOPSize;
        ptEnPara->EnAspectRatio   = tNorConfig->tVideoEnPara_pc[deviceNum][index].EnAspectRatio;
    }
    else
    {
...
load_default_encodepara(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        gtdefaultVideoEnPara[i][index].EnBitrate       = 16000;//kenny test 1800
        gtdefaultVideoEnPara[i][index].EnDeinterlaceOn = MMP_FALSE;
        gtdefaultVideoEnPara[i][index].EnFrameDouble   = MMP_FALSE;
        gtdefaultVideoEnPara[i][index].EnSkipMode      = VIDEO_ENCODER_NO_DROP;
    ...
    void)
{
    ...
    PalMemcpy(&gtConfig.tVideoEnPara_pc, &gtdefaultVideoEnPara, sizeof(gtdefaultVideoEnPara));
